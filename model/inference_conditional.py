...
def train(recognition_model, observation_model, prior_model, optimizer, batches, alfa, device, logger):
    ...
    for N,X in batches:
        ...
        X = X.to(device) # X euclidean coordinates
        N = N.to(device) # Neural Activity

        M = X.shape[0]

        # Generate Z
        mean, log_variance = recognition_model(N,X)
        ...
        mean_N = observation_model(Z)

        # Compute mean of p(z | x) mean and log_std

        mean_z, log_std_z = prior_model(X)

        # Compute Kullback Divergence
        KLD = alfa * (1/M * metric.KLD2(mean, mean_z, log_std, log_std_z))
        log_likelihood = 1/M * metric.gaussian_observation_loss(N, mean_N)

        mse = F.mse_loss(mean_N, N)
...
def test(recognition_model, observation_model, prior_model, batches, device, callback, logger):
    ...
    with torch.no_grad():
        for N,X in batches:
            ...
            # Compute Kullback Divergence
            KLD = 1/M * metric.KLD2(mean, mean_z, log_std, log_std_z)
            log_likelihood = 1/M * metric.gaussian_observation_loss(N, mean_X)
            mse = F.mse_loss(mean_X, N)
        ...
        if callback is not None:
            callback(recognition_model, observation_model, N, mean_X)
...
class Inference:
    ...
    def __init__(self, model, optimizer, device, logger):
        ...
        self.prior_model       = model.prior_model
    ...
    def optimize(self, epochs, data_loader, validate=False, test_loader=None, callback=None):
        ...
        try:
            ...
            for epoch in range(1, epochs+1):
                ...
                metrics = train(recognition_model, observation_model, prior_model, optimizer, data_loader, alfa, device, logger)
                ...
                if validate:
                    test_metrics = test(recognition_model, observation_model, prior_model, test_loader, device, callback, logger)
                    ...
                    print("ELBO Test: {}".format(average_elbo_test))
                    print("KLD Test: {}".format(average_kld_test))
                    print("LogLike Test: {}".format(average_log_likelihood_test))
                    print("MSE Test: {}".format(average_mse_test))
        ...
        # Handle excpetion and save the last computed values
